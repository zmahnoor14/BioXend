...
def _build_cidx_map(
    ...
    if not cidx_map:
        print(
            "[WARN] CIDX map is empty — supply --compounds COMPOUND_MAPPING.tsv "
...
def _resolve_cidx(row: pd.Series, cidx_map: "dict[str, str]") -> str:
    ...
    script still works without --compounds.
    """
    ...
    if common_name:
        print(
            ...
            "Supply --compounds COMPOUND_MAPPING.tsv (from chemicals.py).",
            file=sys.stderr,
        )
...
def main() -> None:
    ...
    compound_mapping = Path(args.compounds) if args.compounds else None
